/*
 * @(#)DisplayMessageAssembler.java 1.00 2006/03/15 Copyright (c) 2006, Stephan
 * Sann 15.03.2006 ssann Vers. 1.0 created
 */
...
public class DisplayMessageAssembler {
	 ...
	 * 
	 * @param part
	 *            Aktuell zu behandelnder Part
	 * @param displayParts
	 *            List, in die die DisplayParts einsortiert werden.
	 * @param multiparts
	 *            Map, in die Multiparts gepackt werden.
	 */
	private static void addDisplayPart(Part part, List<Part> displayParts,
		...
		if (part instanceof BodyPart) {
			...
			if (parent != null) {
				...
				// adden.
				((List<Integer>) multiparts.get(parent)).add(new Integer(
						displayParts.size()));
			}
		}
	...
	private static void deflateMessageParts(Part part, List<Part> displayParts,
		...
		// (Exception "Missing start boundary")
				...
				&& (part.getContentType().toLowerCase().indexOf(
						"rfc822-headers") < 0)) {

			deflateMessageParts((Message) partContent, displayParts,
					inlineParts, multiparts);
		}

		// For content-types that are unknown to the DataHandler system, an
		// input
		...
		// BASE64DecoderStreams duerfen nicht in einen String geparst werden!
			...
			BufferedInputStream bufReader = new BufferedInputStream(
		...
		// 4.) Wenn "Content-Disposition" am Start, muss es "inline" sein.
				...
				&& (((MimeBodyPart) part).getParent().getContentType()
						.toLowerCase().indexOf("multipart/related") >= 0)
				&& ((part.getDisposition() == null) || part.getDisposition()
						.equalsIgnoreCase(Part.INLINE))) {

			inlineParts.put(cleanContentID(((MimePart) part).getContentID()),
					part);
		}
	 ...
	 *            Map, in die Multiparts gepackt werden.
	 * @param e
	 *            Exception, die beim Einlesen geflogen ist
	 */
	private static void assemblePartsForFaultySourceMessage(Message message,
			DisplayMessageModel displayMessage, List<Part> displayParts,
			Map inlineParts, Map multiparts, Exception e)
	...
	public static DisplayMessageModel assembleDisplayMessage(Message message)
		...
		DisplayMessageModel displayMessage = new DisplayMessageModel();
	...
	public static DisplayMessageModel refurbishGivenDisplayMessage(
			DisplayMessageModel displayMessage, Message message)
			throws MessageRetrieveException {
		...
		try {
			...
			try {
				...
				throw (new MessageRetrieveException(
						"Konnte Message-Parts nicht beziehen", e));
			}
		}
