...
public class UiPageBlock extends BizComponent implements IPageBlock {
	...
	@ManyToOne
	@BzFld(name = "界面分类", disabledNavi = true, mode = "c:HM e:HM c1:HM bu:E")
	@Prop("uiCatalog")
	protected UiCatalog catalog;// 每个网站版本的网站由多个页面模版组成

	@ManyToOne
	@BzFld(name = "所属页面", mode = "c:M e:M c1:M bu:E", groupBy = true, masterMapping = true, cascadeMode = "catalog:*:catalog", options = "['type in 0,2']")
	@Prop("uiPage")
	protected UiPage page;

	@ManyToOne
	@BzFld(name = "上级板块", disabledNavi = true, mode = "c1:E", cascadeMode = "page:*:page")
	...
	// ,9:板块容器
	@BzFld(name = "展现方式", disabledNavi = true, options = "0:内置视图,1:自定义,2:页面引用", mode = "*:H v:P")
	@Prop("uiModelType")
	protected byte type;

	@ManyToOne
	@BzFld(name = "视图类型", disabledNavi = true, options = "['type in 0']", mode = "c1:E")
	@Prop("uiModelLib")
	protected UiModelLib viewType;

	@Column(length = 127)
	@BzFld(name = "视图控制器", mode = "c1:E")
	@Prop("uiDataSource")
	protected String viewController;

	@Column(length = 64)
	@BzFld(name = "视图模版", mode = "c1:E")
	@Prop("uiTemplate")
	protected String viewTemplate;

	@Column(length = 512)
	@BzFld(name = "视图表达式", mode = "c1:E")
	protected String viewExpression;

	@ManyToOne
	@BzFld(name = "引用页面", disabledNavi = true, cascadeMode = "catalog:*:catalog", options = "['type in 2']", mode = "c1:E")
	@Prop("uiModelRefPage")
	 ...
	 * 展现 模块数据集
	 */
	@BzFld(name = "展现数据", mode = "c1:E")
	protected Dataset dataset;

	@BzFld(name = "板块布局", mode = "c1:E")
	...
	@BzFld(name = "排版方向", options = "0:纵向(列),1:横向(行)", disabledNavi = true, mode = "c1:E")
	private boolean horizontal;

	@BzFld(name = "行(列)大小", uiTemplate = "ui.widget.field.Spinner", mode = "c1:E")
	private Integer cellCount;

	@BzFld(name = "条目标题长度", uiTemplate = "ui.widget.field.Spinner", mode = "c1:E")
	private Integer titleLength;

	@ManyToOne
	@BzFld(name = "条目链接", disabledNavi = true, options = "['type in 0']", mode = "c1:E")
	protected UiPage link;

	@Column(length = 10)
	@BzFld(name = "条目链接目标", options = "_blank:新窗口,_parent:父窗口", disabledNavi = true, mode = "c1:E")
	protected String linkTarget;

	// 标题选项
	@BzFld(name = "栏目标题", options = "0:隐藏,1:显示", disabledNavi = true, mode = "c1:E")
	private boolean titleEnabled;

	@ManyToOne
	@BzFld(name = "栏目链接", disabledNavi = true, options = "['type in 0']", mode = "c1:E")
	protected UiPage titleLink;

	@Column(length = 10)
	@BzFld(name = "栏目链接目标", options = "_blank:新窗口,_parent:父窗口", disabledNavi = true, mode = "c1:E")
	protected String titleLinkTarget;

	// 图片选项
	@BzFld(name = "图片宽度", disabledNavi = true, uiTemplate = "ui.widget.field.Spinner", mode = "c1:E")
	private Integer imageWidth;

	@BzFld(name = "图片高度", disabledNavi = true, uiTemplate = "ui.widget.field.Spinner", mode = "c1:E")
	private Integer imageHeight;

	// 滚动选项
	@Column(length = 16)
	@BzFld(name = "内容滚动", disabledNavi = true, cascadeMode = "viewType.scrollOptions:1:E", options = "up:向上滚动,left:向左滚动", mode = "c1:E")
	private String scrollDirection;

	@ManyToOne
	@BzFld(name = "引用样式", disabledNavi = true, cascadeMode = "catalog:*:catalog", mode = "c1:E")
	@Prop("uiStyle")
	...
	@BzFld(name = "加载方式", disabledNavi = true, options = "1:异步加载,0:同步加载", mode = "c1:E")
	protected Boolean ajaxLoad;

	@BzFld(name = "图片状态", disabledNavi = true, options = "1:允许空图片,0:禁止空图片", mode = "c1:E")
	protected boolean allowEmptyImg;

	@BzFld(name = "分页按钮", disabledNavi = true, options = "1:显示,0:隐藏", mode = "c1:E", desc = "用于列表类板块，判断是否在列表中显示分页按钮？")
	protected boolean naviButton;

	@BzFld(name = "板块参数", mode = "c1:E")
