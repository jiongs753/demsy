...
, fields = { @BzFld(property = "catalog", gridOrder = 8) //
		...
		, @BzFld(property = "name", name = "模版名称", mode = "c:M e:M", gridOrder = 1)//
		, @BzFld(property = "code", name = "模版编号", gridOrder = 7) //
...
public class UiPage extends BizComponent implements IPage {
	...
	@ManyToOne
	@BzFld(name = "上级分类", cascadeMode = "catalog:*:catalog", options = "['type = 1']")
	protected UiPage parent;

	@ManyToOne
	@BzFld(name = "样式名称", disabledNavi = true, cascadeMode = "catalog:*:catalog")
	protected UiCatalogStyle style;

	@BzFld(name = "页面类型", disabledNavi = true, options = "0:模版,1:分类,2:板块引用", mode = "c:M e:M")
	protected byte type;

	@BzFld(name = "页面用途", disabledNavi = true, cascadeMode = "type:0:E", options = "0:网页模版,1:网站首页,99:后台登录页,100:后台首页")
	protected byte usage;

	@Column(length = 10)
	@BzFld(name = "页面宽度", cascadeMode = "type:0,2:E")
	protected String pageWidth;// 0:表示自动100%

	@Column(length = 10)
	@BzFld(name = "页面高度", cascadeMode = "type:0,2:E")
	...
	@Column(length = 256)
	@BzFld(name = "页面模版", cascadeMode = "type:0,2:E")
	protected String uiTemplate;

	@Column(length = 512)
	@BzFld(name = "关键字", cascadeMode = "type:0:E")
