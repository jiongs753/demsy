...
(function(R, f) {
	...
	if (R[_] && R[_].version >= b)
		return;
	var A = R.NTES, c = R.document, T = R[_] = R.NTES = function($, _) {
		if (!$)
			return $;
		"string" === typeof $ && ($ = L($, _));
		return M($)
	...
	T.util = {
		...
		parseTpl : function(A, $, _) {
			...
			var B = W[A];
			if (!B) {
				B = new Function("obj", "var _=[];with(obj){_.push('"
						+ A.replace(/[\r\t\n]/g, " ").replace(/'(?=[^#]*#>)/g, "\t").split("'").join("\\'").split("\t").join("'").replace(/<#=(.+?)#>/g, "',$1,'").split("<#").join("');").split("#>")
								.join("_.push('") + "');}return _.join('');");
				_ !== false && (W[A] = B)
			}
			return B($)
		},
	...
	var q = R.navigator.userAgent.toLowerCase(), i = /(webkit)[ \/]([\w.]+)/.exec(q) || /(opera)(?:.*version)?[ \/]([\w.]+)/.exec(q) || /(msie) ([\w.]+)/.exec(q) || !/compatible/.test(q)
			&& /(mozilla)(?:.*? rv:([\w.]+))?/.exec(R.navigator.userAgent.toLowerCase());
	T.browser = {};
	if (i) {
		T.browser[i[1] || ""] = true;
