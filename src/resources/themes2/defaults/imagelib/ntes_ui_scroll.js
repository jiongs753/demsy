(function(window, undefined){
	...
	NTES.ui.Scroll = function(body, axis, handle) {
		if (!arguments.length) {
			return;
		}
		...
		if(handle !== undefined){
			...
			var mouse = new NTES.ui.Mouse(t._handle);
			mouse.mouseStart = t._mouseStart.bind(t);
			mouse.mouseDrag = t._mouseDrag.bind(t);
			mouse.mouseStop = t._mouseStop.bind(t);
			NTES(t._hCnt).addEvent('click', t._mouseClick.bind(t));
		}
	};
	
	NTES.ui.Scroll.prototype = {
	...
	NTES.ui.Mouse = function(element) {
		var t = this;
		element = NTES(element);
	...
	NTES.ui.Mouse.prototype = {
		_mouseDown: function(event) {
			...
			NTES(document).addEvent('mousemove', t._mouseMoveDelegate).addEvent('mouseup', t._mouseUpDelegate);
			event.preventDefault();
			return true;
		},
		_mouseMove: function(event) {
			var t = this;
			if (NTES.browser.msie && !event.button) {
		...
		_mouseUp: function(event) {
			var t = this;
			NTES(document).removeEvent('mousemove', t._mouseMoveDelegate).removeEvent('mouseup', t._mouseUpDelegate);
			if (t._mouseStarted) {
