	...
	$.fn.extend({
		accordion : function(options, data) {
			...
			return this.each(function() {
				...
				else if (!$(this).is(".accordion"))
					$.data(this, "accordion", new $.demsy.accordion(this, options));
			});
		},
	});

	$.demsy.accordion = function(container, options) {

		// 合并配置项
		this.options = options = $.extend({}, $.demsy.accordion.defaults, options);
	...
	function toggle(toShow, toHide, data, clickedActive, down) {
		...
		if (options.animated) {
			if (!options.alwaysOpen && clickedActive) {
				$.demsy.accordion.animations[options.animated]({
					toShow : jQuery([]),
					toHide : toHide,
					complete : complete,
					down : down,
