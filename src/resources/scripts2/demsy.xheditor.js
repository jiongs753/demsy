...
var toolsThemes={
	...
	full:'Cut,Copy,Paste,Pastetext,|,Blocktag,Fontface,FontSize,Bold,Italic,Underline,Strikethrough,FontColor,BackColor,SelectAll,Removeformat,|,Align,List,Outdent,Indent,|,Link,Unlink,Anchor,Img,Flash,Media,Hr,Emot,Table,|,Fullscreen'};//,SourcePreview,Print,
toolsThemes.mfull=toolsThemes.full.replace(/\|(,Align)/i,'/$1');
var arrDbClick={'a':'Link','img':'Img','embed':'Embed'},uploadInputname='upload';//DEMSY change filedata to upload
...
var xheditor=function(textarea,options)
{
	...
	//if(!settings.tools.match(/(^|,)\s*About\s*(,|$)/i))settings.tools+=',About';//DEMSY remove About
	...
	this.startUpload=function(fromFiles,toUrl,limitExt,onUploadComplete)
	{
		...
		function onUploadCallback(sText,bFinish)
		{
			...
			//{
				//if(data.customMsg)
				if(!data.success)alert(data.customMsg);
				else{
					arrMsg.push(data.fileUrl);
					bOK=true;//继续下一个文件上传
				}
			//}
