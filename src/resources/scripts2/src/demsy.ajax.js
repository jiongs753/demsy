 ...
 * 
 * form提交后返回json数据结构statusCode=Demsy.statusCode.ok表示操作成功, 做页面跳转等操作. statusCode=Demsy.statusCode.error表示操作失败, 提示错误原因. 
...
/**
 * dialog上的表单提交回调函数
 * 服务器转回navTabId，可以重新载入指定的navTab. statusCode=Demsy.statusCode.ok表示操作成功, 自动关闭当前dialog
 * 
...
function navTabSearch(form, navTabId){
	var $form = $(form);
	if (form[Demsy.pageInfo.pageNum]) form[Demsy.pageInfo.pageNum].value = 1;
...
function _getPagerForm($parent, args) {
	...
	if (form) {
		if (args["pageNum"]) form[Demsy.pageInfo.pageNum].value = args["pageNum"];
		if (args["numPerPage"]) form[Demsy.pageInfo.numPerPage].value = args["numPerPage"];
		if (args["orderField"]) form[Demsy.pageInfo.orderField].value = args["orderField"];
		if (args["orderDirection"] && form[Demsy.pageInfo.orderDirection]) form[Demsy.pageInfo.orderDirection].value = args["orderDirection"];
	}
