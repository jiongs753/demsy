...
Dutils = {
	...
	login : function(title) {
		if ($.pdialog && Durl.login) {
			$.pdialog.open(Durl.login, "login", title || Dnls.login, {
				mask : true,
				width : 520,
				height : 260
			});
		} else {
			window.location = Durl.login;
		}
	},
	/**
	 * 将字符串转换成JSON对象
	...
	ajaxError : function(xhr, ajaxOptions, thrownError) {
		Derror("<div>Http status: " + xhr.status + " " + xhr.statusText + "</div>" + "<div>ajaxOptions: " + ajaxOptions + "</div>" + "<div>thrownError: " + thrownError + "</div>" + "<div>" + xhr.responseText + "</div>");
	},
	/**
	...
	ajaxDone : function(json) {
		...
		// 返回值是错误信息
		if (json.statusCode == Dstatus.error && json.message) {
			Derror(json.message);
		}
		// 返回值是登录超时
		else if (json.statusCode == Dstatus.timeout) {
			Dutils.login();
		} else if (json.message) {
			Dsuccess(json.message);
		}
	},
};
