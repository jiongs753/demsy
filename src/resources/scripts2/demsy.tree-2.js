...
(function($) {
	...
	$.fn.treeview = function(settings) {
		...
		function buildtree(data, ht) {
			...
			ht.push("<ul class='bbit-tree-root ", dfop.theme, "' unselectable='on'>");
		...
		function buildnode(nd, ht, deep, path, isend) {
			...
			ht.push("<li class='bbit-tree-node' unselectable='on'>");
			...
			// span indent
			ht.push("<span class='bbit-tree-node-indent' unselectable='on'>");
			if (deep == 1) {
			...
			if (nd.hasChildren) {
				if (nd.isexpand) {
					ht.push("<ul  class='bbit-tree-node-ct'  unselectable='on' style='z-index: 0; position: static; visibility: visible; top: auto; left: auto;'>");
					if (nd.ChildNodes) {
		...
		function nodeclick(e) {
			...
			if (et.tagName == "IMG") {
				...
				var nid = item.id.replace(/[^\w]/gi, "_");
				if (item.hasChildren) {
					img = $("#" + id + "_" + nid + " img.bbit-tree-ec-icon");
				} else {
					img = $("#" + id + "_" + nid + " img.bbit-tree-node-cb");
				}
				if (img.length > 0) {
					img.click();
				}
		...
		function bindevent() {
			$(this).hover(function() {
				...
				$(this).removeClass("ui-state-hover").addClass("ui-tree-state-default");
